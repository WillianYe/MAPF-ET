...
// TARGET REASONING CAN BE OBTAINED EASILY

// BFS, FS(f=g), FS(f=g+h), EES(f=g+\hat{h})
...
public:
	...
	int getLowerBound() const { return obj_lowerbound; }

	CBS(const Instance& instance, bool sipp, int screen);
	CBS(const Instance& instance, const vector<ConstraintTable>& initial_constraints, bool sipp, int screen);
...
protected:
	...
	list<HLNode*> allNodes_table; // this is used for both ECBS and EES
	...
	// 保持当前上层节点一致路径，持续更新为最新状态
	vector<Path*> paths_consistent;
	// 可能由外部传入或者初始化根节点的时候计算
...
private: // CBS only, cannot be used by ECBS
	...
	pairing_heap< CBSNode*, compare<CBSNode::compare_node_by_d> > focal_list; // this is used for both ECBS and EES
