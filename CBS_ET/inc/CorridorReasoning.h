...
private:
	...
	int getEnteringTime(const std::vector<int>& path, const std::vector<int>& path2, int t);
	int getExitingTime(const std::vector<int>& path, int t);
	int getCorridorLength(const std::vector<int>& path, int t_start, int loc_end, std::pair<int, int>& edge);
