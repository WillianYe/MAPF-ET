...
public:
	...
	// minimizing the number of internal conflicts (that is conflicts with known paths for other agents found so far).
	// lower bound is an underestimation of the length of the path in order to speed up the search.
	// 父类的指针可以指向子类的对象
...
protected:
	...
	//如果采用智能指针，可以直接定义closed_list，不用定义所有节点的列表
	typedef unordered_set<AStarNode*, AStarNode::NodeHasher, AStarNode::eqnode> hashtable_t;
 	hashtable_t allNodes_table;

	// Updates the path data member
	void updatePath(const LLNode* goal, vector<int> &path);
	//更新focal列表
	void updateFocalList();
	// pop node from focal list
	inline AStarNode* popNode();
	// push node into open. if it is with performance bound, push it into focal list
