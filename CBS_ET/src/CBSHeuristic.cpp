...
// 用引用输入和输出参数
void CBSHeuristic::buildCardinalConflictGraph(CBSNode& node, vector<int>& CG, int& num_of_CGedges)
{
	num_of_CGedges = 0;
...
int CBSHeuristic::getEdgeWeight(int a1, int a2, CBSNode& node, bool cardinal)
{
	...
	if (type == heuristics_type::WDG && rst > 0)
	{
		...
		vector<vector<int>> initial_paths(2);
 ...
 * @brief 计算最小顶点覆盖
 *
 * 根据给定的邻接矩阵 CG，计算图的最小顶点覆盖。
 *
 * @param CG 邻接矩阵，表示图中各顶点之间的连接关系
 *
 * @return 返回最小顶点覆盖的大小
 */
 ...
 * @brief 贪心匹配（CBS-h1）
 *
 * 根据给定的一维线性化的数组 CG 和列数 cols，使用贪心算法，按agent id依次进行匹配。
 *
 * @param CG 表示图的邻接矩阵
 * @param cols 列数，即图的节点数
 *
 * @return 匹配成功的边数。返回值为极大匹配，而不是最大匹配。
 */
 ...
 * @brief K顶点覆盖算法
 *
 * 使用K顶点覆盖算法判断给定的图是否存在一个大小为k的顶点覆盖集。
 *
 * @param CG 图的邻接矩阵表示，用vector存储，索引表示顶点，值为边权重（此处使用>0表示有边）
 * @param num_of_CGnodes 图的顶点数
 * @param num_of_CGedges 图的边数
 * @param k 期望的顶点覆盖集大小
 * @param cols 邻接矩阵的列数（即顶点数）
 *
 * @return 如果找到大小为k的顶点覆盖集，返回true；否则返回false
 */
