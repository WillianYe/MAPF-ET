	...
	const vector<Path*>& paths, int endpoints[], int endpoints_time[]) // return the length of the corridor 
{
	...
	endpoints[0] = paths[conflict->a1]->at(endpoints_time[0]); // the exit location for agent 1
	endpoints[1] = paths[conflict->a2]->at(endpoints_time[1]); // the exit location for agent 2
	...
	int curr = paths[conflict->a1]->at(endpoints_time[0] - 1);
	...
	const vector<Path*>& paths, const HLNode& node)
{
	...
	t3 = search_engines[conflict->a1]->getTravelTime(paths[conflict->a1]->front(), u[1], ct1, MAX_TIMESTEP);
	...
	t3_ = search_engines[conflict->a1]->getTravelTime(paths[conflict->a1]->front(), u[1], ct1, t3 + 2 * corridor_length + 1);
	ConstraintTable ct2(initial_constraints[conflict->a2]);
    ct2.insert2CT(node, conflict->a2);
	t4 = search_engines[conflict->a2]->getTravelTime(paths[conflict->a2]->front(), u[0], ct2, MAX_TIMESTEP);
	...
	t4_ = search_engines[conflict->a2]->getTravelTime(paths[conflict->a2]->front(), u[0], ct2, t3 + corridor_length + 1);
...
int CorridorReasoning::getExitingTime(const std::vector<int>& path, int t)
{
	if (t >= (int)path.size())
		t = (int)path.size() - 1;
	int loc = path[t];
...
int CorridorReasoning::getEnteringTime(const vector<int>& path, const vector<int>& path2, int t)
{
	if (t >= (int)path.size())
		t = (int)path.size() - 1;
	int loc = path[t];
	while (loc != path.front() && loc != path2.back() &&
		search_engines[0]->instance.getDegree(loc) == 2)
	{
		t--;
		loc = path[t];
...
int CorridorReasoning::getCorridorLength(const vector<int>& path, int t_start, int loc_end, pair<int, int>& edge)
{
	int curr = path[t_start];
	int next;
	int prev = -1;
...
bool CorridorReasoning::blocked(const Path& path, const Constraint& constraint)
{
	...
	for (int t = t1; t < t2; t++)
	{
		if (t >= (int)path.size() && loc == path.back())
			return true;
		else if (t >= 0 && path[t] == loc)
			return true;
	}
	return false;
}