			...
			while (foundBypass)
			{
				...
				for (int i = 0; i < 2; i++)
				{
					...
					if (!solved[i])
					{
						...
						child[i]->sum_of_costs <= suboptimality * obj_lowerbound &&
...
void ECBS::adoptBypass(ECBSNode* curr, ECBSNode* child, const vector<int>& fmin_copy)
{
	...
	for (const auto& path : child->paths) // update paths
	{
		...
		if (p == curr->paths.end())
		{
			...
			paths_consistent[path.first] = &curr->paths.back().second.first;
...
void ECBS::updatePaths(ECBSNode* curr)
{
	...
	while (curr != nullptr)
	{
		for (auto & path : curr->paths)
		{
			...
			if (!updated[agent])
			{
				paths_consistent[agent] = &path.second.first;
...
bool ECBS::generateRoot()
{
	...
	for (auto i : agents)
	{
		paths_found_initially[i] = search_engines[i]->findSuboptimalPath(*root, initial_constraints[i], paths_consistent, i, 0, suboptimality);
...
{
	clock_t t = clock();
	auto new_path = search_engines[ag]->findSuboptimalPath(*node, initial_constraints[ag], paths_consistent, ag, min_f_vals[ag], suboptimality);
...
inline void ECBS::pushNode(ECBSNode* node)
{
	...
	case high_level_solver_type::ASTAREPS:  // cleanup_list is called open_list in ECBS
        if (node->sum_of_costs <= suboptimality * obj_lowerbound)
...
ECBSNode* ECBS::selectNode()
{
	...
	case high_level_solver_type::EES:
		...
		if (focal_list.top()->sum_of_costs <= suboptimality * obj_lowerbound)
		{ // return best d
			curr = focal_list.top();
		...
		else if (open_list.top()->sum_of_costs <= suboptimality * obj_lowerbound)
		{ // return best f_hat
			curr = open_list.top();
...
void ECBS::classifyConflicts(ECBSNode &node)
{
	...
	while (!node.unknownConf.empty())
	{
		...
		if (rectangle_reasoning &&
		    (int)paths_consistent[a1]->size() - 1 == min_f_vals[a1] && // the paths for both agents are their shortest paths
		    (int)paths_consistent[a2]->size() - 1 == min_f_vals[a2] &&
