...
Instance::Instance(const string& map_fname, const string& agent_fname, int num_of_agents, 
	int due_time_addend, int num_of_rows, int num_of_cols, int num_of_obstacles, 
    int warehouse_width):
	map_fname(map_fname), agent_fname(agent_fname), num_of_agents(num_of_agents), due_time_addend(due_time_addend)
{
	bool succ = loadMap();
	if (!succ)
	{
...
bool Instance::loadMap()
{
	...
	if (!myfile.is_open())
		return false;
	string line;
	// boost::tokenizer可以方便地将一个字符串分割成多个子串
	tokenizer< char_separator<char> >::iterator beg;
	// std::getline() 用于从输入流中读取一行文本的标准库函数.函数会丢弃分隔符，即不会将分隔符包含在返回的字符串中
	getline(myfile, line);
	...
	else // my benchmark, randomly generated
	{
		char_separator<char> sep(",");
		tokenizer< char_separator<char> > tok(line, sep);
		beg = tok.begin();
...
bool Instance::loadAgents()
{
	...
	if (line[0] == 'v') // Nathan's benchmark
	{
		...
		start_locations.resize(num_of_agents);
		goal_locations.resize(num_of_agents);
		due_times.resize(num_of_agents);
		char_separator<char> sep("\t");
		for (int i = 0; i < num_of_agents; i++)
		{
			...
			beg++;
			beg++;
			due_times[i] = atoi((*beg).c_str())+due_time_addend;
		}
	}
	else // My benchmark
	{
		...
		for (int i = 0; i<num_of_agents; i++)
		{
			...
			goal_locations[i] = linearizeCoordinate(row, col);
			c_beg++;
			due_times[i] = atoi((*c_beg).c_str())+due_time_addend;
		}
	}
	myfile.close();
	return true;
...
void Instance::printAgents() const
{
  for (int i = 0; i < num_of_agents; i++) 
  {
    cout << "Agent" << i << " : S=(R" << getRowCoordinate(start_locations[i]) << ",C" << getColCoordinate(start_locations[i]) 
				<< ") ; G=(R" << getRowCoordinate(goal_locations[i]) << ",C" << getColCoordinate(goal_locations[i]) << ")"
				<< "; DT=" << due_times[i] << ""
				 << endl;
  }
}
...
void Instance::saveAgents() const
{
    ...
    string output=map_fname;
    size_t lastSlashPos = map_fname.find_last_of('/');
    if (lastSlashPos != std::string::npos) {
        output = map_fname.substr(lastSlashPos + 1);
    }
    myfile << "version 1" << endl;
    for (int i = 0; i < num_of_agents; i++)
        myfile << "0\t" << output << "\t" << num_of_cols << "\t" << num_of_rows << "\t" << getColCoordinate(start_locations[i]) << "\t" << getRowCoordinate(start_locations[i]) << "\t" 
