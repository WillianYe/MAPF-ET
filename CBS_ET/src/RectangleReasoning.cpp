...
{
	assert(timestep > 0);
	int from1 = (*paths[a1])[timestep - 1];
	int from2 = (*paths[a2])[timestep - 1];
	int loc = paths[a1]->at(timestep);
	...
	bool haveBarriers = ExtractBarriers(*mdd1, loc, timestep, loc - from1, loc - from2, 
		paths[a1]->at(t_start), paths[a1]->at(t_end), t_start, B1);
	if (!haveBarriers)
		return nullptr;
	...
	haveBarriers = ExtractBarriers(*mdd2, loc, timestep, loc - from2, loc - from1,
		paths[a2]->at(t_start), paths[a2]->at(t_end), t_start, B2);
	if (!haveBarriers)
		return nullptr;
        ...
        int a1, int a2, const MDD* mdd1, const MDD* mdd2)
{
	...
	pair<int, int> location = instance.getCoordinate(paths[a1]->at(timestep));
    ...
    for (int t1_start : s1s)
    {
        for (int t1_end : g1s)
        {
			auto s1 = instance.getCoordinate(paths[a1]->at(t1_start));
			auto g1 = instance.getCoordinate(paths[a1]->at(t1_end));
            ...
            for (int t2_start : s2s)
            {
                for (int t2_end : g2s)
                {
                    auto s2 = instance.getCoordinate(paths[a2]->at(t2_start));
                    auto g2 = instance.getCoordinate(paths[a2]->at(t2_end));
...
bool RectangleReasoning::blockedNodes(const vector<int>& path, 
...
void RectangleReasoning::generalizedRectangle(const vector<int>& path1, const vector<int>& path2, 
...
{
	int loc = path1[timestep];
	int dir1 = loc - path1[timestep - 1];
	int dir2 = loc - path2[timestep - 1];
...
list<int> RectangleReasoning::getStartCandidates(const Path& path, const MDD& mdd, int timestep)
{
	...
	for (int t = 0; t <= timestep; t++) //Find start that is single and Manhattan-optimal to conflicting location
	{
		if (mdd.levels[t].size() == 1 &&
			mdd.levels[t].front()->location == path[t] &&
			instance.getManhattanDistance(path[t], path[timestep]) == timestep - t)
			starts.push_back(t);
	}
	return starts;
}
...
list<int> RectangleReasoning::getGoalCandidates(const Path& path, const MDD& mdd, int timestep)
{
	...
	for (int t = (int) path.size() - 1; t >= timestep; t--) //Find end that is single and Manhattan-optimal to conflicting location
	{
		if (mdd.levels[t].size() == 1 &&
			mdd.levels[t].front()->location == path[t] &&
			instance.getManhattanDistance(path[t], path[timestep]) == t - timestep)
			goals.push_back(t);
	}
	return goals;
}
...
int RectangleReasoning::getStartCandidate(const Path& path, int dir1, int dir2, int timestep)
{
	...
	{
		if (path[t] - path[t - 1] != dir1 && path[t] - path[t - 1] != dir2)
			return t;
	}
	return 0;
}
...
int RectangleReasoning::getGoalCandidate(const Path& path, int dir1, int dir2, int timestep)
{
	...
	{
		if (path[t + 1] - path[t] != dir1 && path[t + 1] - path[t] != dir2)
			return t;
	}
	return (int)path.size() - 1;
}

/*int getRectangleTime(const Conflict& conflict, const std::vector<std::vector<int>*>& paths, int num_col)
{
	int s1 = paths[conflict.a1]->at(std::get<3>(conflict));
	int s2 = paths[conflict.a2]->at(std::get<4>(conflict));
...
{
	if (t >= (int)path.size())
		return loc == path.back();
	else return t >= 0 && path[t] == loc;
}
