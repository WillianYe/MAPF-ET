...
void SpaceTimeAStar::updatePath(const LLNode* goal, vector<int> &path)
{
    const LLNode* curr = goal;
    if (curr->is_goal)
        curr = curr->parent;
    // 将向量的容量（向量无需重新分配即可容纳的元素总数）增加到大于或等于 new_cap 的值。如果 new_cap 大于当前 capacity() ，则分配新存储，否则该函数不执行任何操作。reserve() 不改变向量的大小。
...
// lowerbound is an underestimation of the length of the path in order to speed up the search（外部可利用的启发式信息）
                                                   ...
                                                   const vector<Path*>& paths, int agent, int lowerbound, double w)
{
    ...
    while (!open_list.empty())
    {
        ...
        if (curr->location == goal_location && // arrive at the goal location
            ...
            // 因为路径长度约束，路径可能包含多个goal节点。需要保证agent到达goal之后的多段时间（为true）并变为false之后，才能算成功
        ...
        for (int next_location : next_locations)
        {
            ...
            //若为已存在节点，则按情况更新/加入focal，按情况更新open
