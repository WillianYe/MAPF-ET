classdef MILPModel<handle
    ...
    methods
        function obj = MILPModel(network,robotNum,objectiveSelect)
            switch objectiveSelect
                ...
                case 8   % total ET
                    ...
                    f = obj.getObjectiveTotalET(network,robotNum);
                    AWidth = network.ArcRobotPairNum + network.T*robotNum;
                ...
                case 9 % maximum ET
                    ...
                    constraint7 = obj.getConstraintLogicOperationEqual(network,robotNum); %equality
                    constraint8 = obj.getConstraintLateness(network,robotNum); %inequality
                    constraint9 = obj.getConstraintEarliness(network,robotNum); %inequality
                    %和maximum lateness的区别，还要查看最小值等
                    ...
                    AineqI = [AineqI;AineqSize + constraint9.AI];
                    AineqJ = [AineqJ; constraint9.AJ];
                    AineqV = [AineqV; constraint9.AV];
                    bineq = [bineq; constraint9.b];
                    AineqSize = AineqSize + constraint9.Num;
        ...
        %% constraint earliness, inequality
        function constraint = getConstraintEarliness(obj,network,robotNum)
            AI = zeros(robotNum*2*network.T,1);
            AJ = zeros(robotNum*2*network.T,1);
            AV = ones(robotNum*2*network.T,1);
            b = zeros(robotNum,1);
            ...
            for i=1:robotNum
                ...
                b(i,1) = -network.GoalRCT(i,3)+network.T+1;%constraint lateness负号
            end
        ...
        %% objective: total ET
        function f = getObjectiveTotalET(obj,network,robotNum)
            f = zeros(1,network.ArcRobotPairNum+network.T*robotNum);
            for t=1:network.T
                for i=1:robotNum
                    if t <= network.GoalRCT(i,3)
                        val = -1;%和total tardiness的唯一区别！
                    else
                        val = 1;
                    end
                    f(1,network.ArcRobotPairNum+(t-1)*robotNum+i) = val;
